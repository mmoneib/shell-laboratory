...
* **Archetypical Categorization**: The most basic and essential categorization in Shell Laboratory is based on archetypes rather than types. Classification into these categories is based on the degree of reusability and the type of input/output operations of the code in question. There are 3 main archetypes: actions, utilities, and processes. The Archetypical Categorization defines the type of a script file in the repository as denoted by the postfix starting with the double underscore.
...
* **Process**: Scripts which are meant to be user-facing and performing several functions. A process should use utilities to store and manipulate data structure, and actions to perform common functionalities. Processes are strictly Procedural and they consist of 3 code sections: input, process, and output, all accessing a shared memory space with the parameters designated by their prefixes indicating from which section they originate. This allows for clean yet flexible flow of data across the process.
...
2. While the whole script uses a form of shared memory,it is softly partitioned by prefixes as follows:
  - Input parameters are used only for configuration, to be accessed only by the input function, and are prefixed by c\_r for required ones and c\_o for optional ones.
  - Process parameters are prepared by the input function and accessed only by the process function; they are prefixed by d\_.
  - Output parameters are prepared by the process function and accessed only by the output function; they are prefixed by o\_.
3. Each optional or required input parameter should have an inline explanatory comment above it. 
